  ...
  <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;padding:20px 24px;border-radius:0 0 8px 8px">
    ...
    sendWeekly: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const [vs] = await db.select().from(venueSettings).where(eq(venueSettings.ownerId, ctx.user.id)).limit(1);
        if (!vs?.smtpHost || !vs?.smtpUser || !vs?.smtpPass) throw new Error('SMTP not configured — add email settings in Settings → Email.');

        // Build week date range (Mon 00:00 → Sun 23:59 in venue timezone)
        const tz = vs.timezone || 'Pacific/Auckland';
        const weekStartDate = new Date(`${input.weekStart}T00:00:00`);
        const weekEndDate = new Date(weekStartDate);
        weekEndDate.setDate(weekEndDate.getDate() + 6);
        ...
        const dayColors = ['#1a3a2a','#2d5a3d','#1a3a2a','#2d5a3d','#1a3a2a','#2d5a3d','#1a3a2a'];
        let dayBlocksHtml = '';
        ...
        for (const [dayKey, dayBookings] of Array.from(byDay.entries()).sort()) {
          ...
          for (const booking of dayBookings) {
            ...
            // Load runsheet (latest one for this booking)
            const [sheet] = await db.select().from(runsheets)
              .where(and(eq(runsheets.bookingId, booking.id), eq(runsheets.ownerId, ctx.user.id)))
              .orderBy(runsheets.createdAt)
              .limit(1);
            ...
            if (sheet) {
              const items = await db.select().from(runsheetItems)
                .where(eq(runsheetItems.runsheetId, sheet.id))
              ...
              // F&B items
              const fnb = await db.select().from(fnbItemsTable)
                .where(and(eq(fnbItemsTable.runsheetId, sheet.id), eq(fnbItemsTable.ownerId, ctx.user.id)));
              if (fnb.length) {
                const foodItems = fnb.filter(f => (f.course ?? '') !== 'Drinks');
                const drinkItems = fnb.filter(f => (f.course ?? '') === 'Drinks');
                const renderFnbRows = (items: typeof fnb) => items.map(f =>
                ...
                if (foodItems.length) {
                  fnbHtml += `<tr><td colspan="2" style="padding:6px 0 2px;font-size:11px;font-weight:bold;letter-spacing:1.5px;text-transform:uppercase;color:#6b7280">FOOD</td></tr>` + renderFnbRows(foodItems);
                }
                if (drinkItems.length) {
                  fnbHtml += `<tr><td colspan="2" style="padding:6px 0 2px;font-size:11px;font-weight:bold;letter-spacing:1.5px;text-transform:uppercase;color:#6b7280">DRINKS</td></tr>` + renderFnbRows(drinkItems);
                }
              }
            }
        ...
        const weekStartFormatted = fmtDate(weekStartDate);
        const weekEndFormatted = fmtDate(weekEndDate);
        const venueName = vs.name ?? 'Your Venue';
        const subject = input.subject || `Staff Runsheets — Week of ${input.weekStart}`;
        ...
        const html = `
          <div style="font-family:sans-serif;max-width:700px;margin:0 auto;background:#f7f3ee;padding:24px">
            <div style="background:#1a3a2a;color:#f5e6c8;padding:20px 24px;border-radius:8px 8px 0 0">
              <div style="font-size:11px;letter-spacing:3px;text-transform:uppercase;opacity:0.7">${venueName}</div>
              <div style="font-size:22px;font-weight:bold;margin-top:4px">Staff Runsheets</div>
              <div style="font-size:13px;opacity:0.8;margin-top:2px">Week of ${weekStartFormatted}</div>
            </div>
            <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;padding:20px 24px;border-radius:0 0 8px 8px">
        ...
        await transporter.sendMail({
          from: `"${fromName}" <${fromEmail}>`,
          to: input.to,
          replyTo: vs.notificationEmail ?? fromEmail,
          subject,
          html,
          text: `Staff Runsheets — Week of ${input.weekStart}\n\n${allBookings.map(b => {
            const d = b.eventDate instanceof Date ? b.eventDate : new Date(b.eventDate as any);
            return `${fmtDate(d)} ${fmtTime(d)} — ${[b.firstName, b.lastName].filter(Boolean).join(' ')} (${b.guestCount ?? '?'} pax) — ${b.spaceName ?? ''}`;
          }).join('\n')}`,
        });
        return { success: true, eventCount: allBookings.length };
      }),
  ...
  runsheets: router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        dietaries: z.array(z.object({ name: z.string(), count: z.number(), notes: z.string().optional(), names: z.array(z.string()).optional() })).optional(),
